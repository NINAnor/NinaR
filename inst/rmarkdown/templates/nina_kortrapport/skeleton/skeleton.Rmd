...
Dette er et eksempel på en NINA kortrapport laget direkt fra statistikkprogrammet R, gjennom packen `NinaR` [@NinaR]. Den kan med fordel brukes da rapporten har stor innhold av R-skript. Bortsett fra avsnitten med kod er mallen tenkt å efterligne NINAs standardmalle for kortrapport. Mallen brukes på samme måte som andre maller i pakken `Rmarkdown` og `rticles` (se http://rmarkdown.rstudio.com/) . Rstudio er særskilt passende å bruke, men mallen kan også brukes direkte i R (for eksempel gjennom terminalen/Command Prompt).

Fremgangsmåte
-------------

_For å bruke mallen trengs:_
...
Når R-pakken `NinaR` er installert, finnes mallen i Rstudio gjennom `File -> New File -> R Markdown -> From templates`. Alternativt kan en mall produseres gjennom `rmarkdown::draft("title", template="nina_kortrapport", package="NinaR")`.
  
Høyst oppe i mallen (vises ikke i PDFen) finns en såkalt "YAML-seksjon" der diverse obligatoriske ting skal skrives in. En lukket PDF kan lages gjennom å skrive "yes" (eller hva som helst) etter secure_pdf. 

Referenser kan inkluderes på to måter. Vi kan for eksempel referere til @Pedersen2016 i teksten, eller ikke [@Pedersen2016]. 

Dette templat baseres på R-pakken `Rmarkdown` [@rmarkdown], se for eksempel http://rmar\-kdown.rstudio.com/. Nina-pakken `NinaR` bruker kod fra @rticles for templater. 

Ny side, hvis det ønskes for eksempel mellom ulike kapitel, lages gjennom å skrive `\newpage`.

Spørsmål og synspunkter kan sendes til Jens Åström.
\newpage
