...
#'
#'tmp2 <- grassDailyClimate(points = points, start_time = "2013-01-01", end_time = "2014-12-31",
...
grassDailyClimate <- function(points, variables=c("Temperature", "Precipitation"), start_time, end_time, where=NULL, nprocs=10){
  ...
  rgrass7::execGRASS("g.region", align = "precipitation_1957_01_01@gt_Meteorology_Norway_seNorge_Precipitation_days",
  ...
  for(v in variables){
    ...
    rgrass7::execGRASS("g.mapsets", operation = "add", mapset = paste0("gt_Meteorology_Norway_seNorge_", v, "_days"))

    cat("This can take some time...")
    strds_var <- paste(tolower(substr(v, 1, 1)), substr(v, 2, nchar(v)), sep="")
    variable_daily <- rgrass7::execGRASS("t.rast.what", flags=c("n", "i", "overwrite", "verbose"),
